...
func TestVerifyHandler_LoginSendConfirm(t *testing.T) {
	...
	e := VerifyHandler{
		...
		Issuer:   "iss-test",
		L:        logger.Std,
		Sender:   provider.SenderFunc(emailer.Send),
...
func TestVerifyHandler_LoginSendConfirmEscapesBadInput(t *testing.T) {
	...
	site := "/login?address=blah@user.com&user=" + url.QueryEscape(badData) + "&site=" + url.QueryEscape(badData)
	req, err := http.NewRequest("GET", site, http.NoBody)
	require.NoError(t, err)
	handler.ServeHTTP(rr, req)
	assert.Equal(t, 200, rr.Code)
	assert.Equal(t, "blah@user.com", emailer.to)
