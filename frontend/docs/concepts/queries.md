...
| Сценарий | Статус |
|----------|--------|
| Показать skeleton при первой загрузке | `isPending` |
| Показать spinner при рефетче (данные уже есть) | `isFetching && !isPending` |
| Заблокировать UI пока нет данных | `isPending` |

В коде проекта используем `isPending` — в том числе в собственных хуках вроде `useAuth()`,
которые прокидывают статус запроса наружу. `isLoading` не используем нигде.
